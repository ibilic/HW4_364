...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:si364@localhost/hw4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class User(UserMixin, db.Model):
    ...
    user_gifs = db.relationship('PersonalGifCollection', backref='load_user')
...
class SearchTerm(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)# id (Integer, primary key)
    term = db.Column(db.String(32), unique=True, index=True)# term (String, up to 32 characters, unique) -- You want to ensure the database cannot save non-unique search terms
...
def get_gifs_from_giphy(search_string):
    ...
    url = baseurl + "?q=" + search_string  + "&api_key=" + api_key + "&limit=5"
    resp = requests.get(url)
    text = resp.text
    python_obj = json.loads(text)
    # return str(python_obj['data'])
    list_tuples = []
    for info in python_obj['data']:
        y = info['title']
        x = info['embed_url']
        tup = (y,x)
...
def get_or_create_gif(db_session, title, url):
    ...
    #Determining whether the gif already exists in the database should be based on the gif's title.
...
def get_or_create_search_term(db_session, term, gif_list = []):
    ...
    if searchTerm:
        ...
        gif_list = get_gifs_from_giphy(str(searchTerm))
        for info in gif_list:
            print (type(info))
            gif = get_or_create_gif(db_session, title = info[0], url = info[1])
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    # TODO 364: This function should get or create a personal gif collection.
    #Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.

    # In other words, based on the input to this function, if there exists a collection with the input name,
    #associated with the current user, then this function should return that PersonalGifCollection instance.

    # However, if no such collection exists, a new PersonalGifCollection instance should be created, and
    #each Gif in the gif_list input should be appended to it (remember, there exists a many to many relationship
    #between Gifs and PersonalGifCollections).
...
def index():
    ...
    form = GifSearchForm()
    if form.validate_on_submit():
        searchterm = get_or_create_search_term(db.session, form.search.data)
...
def create_collection():
    ...
    # TODO 364: If the form validates on submit, get the list of the gif ids that were selected from
    #the form. Use the get_gif_by_id function to create a list of Gif objects.  Then, use the
    #information available to you at this point in the function (e.g. the list of gif objects,
    #the current_user) to invoke the get_or_create_collection function, and redirect to the page that
    #shows a list of all your collections.
    ...
    if request.method == 'POST':
        ...
        get_or_create_collection(db.session,current_user=current_user,name=form.name.data,gif_list=gif_objects) # How to access user, here and elsewhere TODO
        flash('Collection Made')
...
def collections():
    ...
    # TODO 364: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template.
    #Make sure to examine the template so that you send it the correct data!
